...
public class StorageDirectory : IStorageDirectory
{
    ...
    public StorageDirectory(IConfiguration configuration)
    {
        ...
        _root = StorageHelper.MapPath(path);
    ...
    {
        return CreateStorageFile(GetPhysicalPath(path), path);
    }
    ...
    public virtual async Task<IStorageFile> SaveAsync(string text, string directoryName, string fileName)
    {
        ...
        return CreateStorageFile(path, Path.Combine(directoryName, fileName));
    }
    ...
    /// <param name="absolutePath">访问地址。</param>
    /// <returns>返回实例化的文件存储实例。</returns>
    protected IStorageFile CreateStorageFile(string path, string absolutePath) => new StorageFile(path, absolutePath);
