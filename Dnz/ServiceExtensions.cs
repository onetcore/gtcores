...
namespace Dnz
{
    ...
    public static class ServiceExtensions
    {
        ...
        public static IServiceBuilder AddDnz(this IServiceCollection services, IConfiguration configuration)
        {
        ...
        private static bool IsDnzDependency(this RuntimeLibrary library)
        {
            if (library.Name == "Dnz" || library.Name.StartsWith("Dnz.") || library.Type == "project")
                return true;
            return library.Dependencies.Any(x => x.Name == "Dnz" || x.Name.StartsWith("Dnz."));
        }
        ...
        public static IEnumerable<Assembly> GetAssemblies(this IConfiguration configuration)
        {
            ...
            {
                if (excludes.Contains(library.Name, StringComparer.OrdinalIgnoreCase) || !library.IsDnzDependency())
                {//尽量减少不需要的程序集
                    continue;
                }
        ...
        public static IServiceProvider BuildServiceProvider(this IConfiguration configuration, Action<IServiceBuilder>? action = null)
        {
            ...
            var builder = services.AddDnz(configuration);
