...
public class ResxCommandHandler : CommandHandler
{
    /// <summary>
    /// 描述。
    /// </summary>
    public override string Description => "生成资源文件的Resources.Designer.cs文件。";
    ...
    public override async Task ExecuteAsync(CommandArgs args, CancellationToken token = default)
    {
        ...
        if (args.ContainsKey("d"))
        {
            ...
            if (dir == null)
            {
                ConsoleCore.ErrorLine("目录不能为空。");
                return;
            }
            var path = Path.Combine(currentDirectory.FullName, dir!);
            ...
            {
                ConsoleCore.ErrorLine($"目录不存在: {path}");
                return;
            }
        }
        else if (args.ContainsKey("p"))
        {
            ...
            {
                ConsoleCore.ErrorLine($"项目不存在: {projectName}");
                return;
            }
        }
...
");
            foreach (XmlElement element in data)
            {
                ...
                await item.WriteAsync(writer, scope);
            }
            // 写入结束符
            await writer.WriteLineAsync(@"
    }
    ...
    {
        ShowLine("[-p project] [-d path] [-public]");
        ShowLine("[-p project]", "指定项目名称，默认当前目录下的*.csproj文件。");
        ShowLine("[-d path]", "指定资源文件所在的目录，默认当前目录。");
        ShowLine("[-public]", "公开资源类，默认internal。");
    }

    private IEnumerable<ResxFileInfo> LoadResources(DirectoryInfo solutionDir)
    {
    ...
    class ResxItem
    {
        ...
        public async Task WriteAsync(StreamWriter writer, string scope)
        {
            await writer.WriteLineAsync($@"        
        /// <summary>
