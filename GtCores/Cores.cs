...
public static class Cores
{
    ...
    {
        const string salt = "AmazingHashedCodeForGtCores!";
    ...
    /// <param name="values">实例列表。</param>
    /// <param name="action">执行方法。</param>
    /// <param name="catchException">是否捕获错误实例。</param>
    public static void ForEach<T>(this IEnumerable<T> values, Action<T> action, bool catchException = true)
    {
        if (catchException)
        {
            foreach (var value in values)
            {
                try { action(value); } catch { }
            }
        }
        else
        {
