...
/// 电子邮件管理接口。
/// </summary>
public interface IEmailManager : IPagableObjectManager<Email>
{
    /// <summary>
    /// 获取资源，一般为内容。
    /// </summary>
    ...
    /// <param name="action">实例化方法。</param>
    /// <returns>返回发送结果。</returns>
    bool SendEmail(User user, string resourceKey, object? replacement = null, Type? resourceType = null, Action<Email>? action = null);
    ...
    /// <param name="action">实例化方法。</param>
    /// <returns>返回发送结果。</returns>
    Task<bool> SendEmailAsync(User user, string resourceKey, object? replacement = null, Type? resourceType = null, Action<Email>? action = null);
